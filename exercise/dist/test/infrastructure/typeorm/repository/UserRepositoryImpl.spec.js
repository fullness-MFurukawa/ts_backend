...
describe('UserRepositoryImplの単体テスト', () => {
    ...
    describe('findById() メソッド', () => {
        it('存在するユーザーIDでユーザーを取得できる', async () => {
            const userId = UserId_1.UserId.fromString('3d1e89a4-06dc-11f0-9fce-6a0ec65304f1');
        ...
        it('存在しないユーザーIDでnullが返される', async () => {
            const userId = UserId_1.UserId.fromString('3d1e89a4-06dc-11f0-9fce-6a0ec65304f2');
            const user = await repository.findById(userId);
    ...
    describe('findByUsername() メソッド', () => {
        it('存在するユーザー名でユーザーを取得できる', async () => {
            const username = Username_1.Username.fromString('testuser');
            const user = await repository.findByUsername(username);
            expect(user).not.toBeNull();
            expect(user.getEmail().getValue()).toBe('testuser@example.com');
        });
        it('存在しないユーザー名でnullが返される', async () => {
            const username = Username_1.Username.fromString('nonexistentuser');
            const user = await repository.findByUsername(username);
            expect(user).toBeNull();
        });
    });
    describe('findByEmail()', () => {
        it('存在するメールアドレスでユーザーを取得できる', async () => {
            const email = Email_1.Email.fromString('testuser@example.com');
            const user = await repository.findByEmail(email);
            expect(user).not.toBeNull();
            expect(user.getUsername().getValue()).toBe('testuser');
        });
        it('存在しないメールアドレスでnullが返される', async () => {
            const email = Email_1.Email.fromString('nobody@example.com');
            const user = await repository.findByEmail(email);
            expect(user).toBeNull();
        });
    });
    describe('existsByUsername()', () => {
        ...
        it('存在しないユーザー名に対してfalseを返す', async () => {
            const username = Username_1.Username.fromString('nouser123');
            const exists = await repository.existsByUsername(username);
    ...
    describe('existsByEmail() メソッド', () => {
        ...
        it('存在しないメールアドレスに対してfalseを返す', async () => {
            const email = Email_1.Email.fromString('notfound@example.com');
            const exists = await repository.existsByEmail(email);
            expect(exists).toBe(false);
        });
    });
    describe('create()', () => {
        it('新規ユーザーを登録し取得できる', async () => {
            ...
            const username = Username_1.Username.fromString('createduser');
            const email = Email_1.Email.fromString('createduser@example.com');
            const password = Password_1.Password.fromHashed('hashedpassword123');
            const roles = [
                new Role_1.Role(RoleId_1.RoleId.fromString('3d1e3fd4-06dc-11f0-9fce-6a0ec65304f1'), RoleName_1.RoleName.fromString('user'))
            ];
            const user = User_1.User.createNew(username, password, email, roles);
            try {
    ...
    describe('updateById()', () => {
        it('既存のユーザーのメールアドレスとパスワードを更新できる', async () => {
            ...
            try {
                // 既存ユーザーを取得
                const original = await repository.findByUsername(Username_1.Username.fromString('testuser'), entityManager);
                expect(original).not.toBeNull();
                // メールアドレスとパスワードを変更
                const newEmail = Email_1.Email.fromString('updated@example.com');
                const newPassword = Password_1.Password.fromHashed('updatedhashedpassword');
        ...
        it('存在しないユーザーIDでは更新できず、falseを返す', async () => {
            ...
            try {
                // 存在しないユーザーIdでダミー作成
                const user = User_1.User.restore(UserId_1.UserId.fromString('3d1f9999-06dc-11f0-9fce-6a0ec65304f2'), Username_1.Username.fromString('nonexistent'), Password_1.Password.fromHashed('dummy'), Email_1.Email.fromString('nonexistent@example.com'), IsActive_1.IsActive.active(), CreatedAt_1.CreatedAt.now(), UpdatedAt_1.UpdatedAt.now(), []);
    ...
    describe('deleteById()', () => {
        it('既存のユーザーを削除できる', async () => {
            ...
            try {
                // 既存ユーザーを取得
                const existing = await repository.findByUsername(Username_1.Username.fromString('testuser'), entityManager);
                expect(existing).not.toBeNull();
                // 削除処理
                const result = await repository.deleteById(existing.getId(), entityManager);
                expect(result).toBe(true);
                // 削除確認
                const deleted = await repository.findById(existing.getId(), entityManager);
        ...
        it('存在しないユーザーIDは削除できずfalseを返す', async () => {
            ...
            const entityManager = queryRunner.manager;
            try {
                const result = await repository.deleteById(UserId_1.UserId.fromString('3d1f9999-06dc-11f0-9fce-6a0ec65304f2'), entityManager);
                expect(result).toBe(false);
            }
            finally {
                await queryRunner.rollbackTransaction();
