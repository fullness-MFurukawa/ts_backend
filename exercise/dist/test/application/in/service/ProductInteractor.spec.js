...
describe('ProductInteractorの単体テスト', () => {
    ...
    beforeAll(async () => {
        ...
        await app.init();
        // ユースケースを取得
        usecase = app.get('ProductUsecase');
    });
    /**
     * すべてのテストの後処理
     */
    ...
    it('存在しない商品Idの場合、NotFoundExceptionがスローされる', async () => {
        ...
        // エラーをスローすることを検証
        await expect(usecase.getByProductId(id)).rejects.toThrow(NotFoundException_1.NotFoundException);
        await expect(usecase.getByProductId(id)).rejects.toThrow(`商品Id:(ac413f22-0cf1-490a-9635-7e9ca810e545)の商品は存在しません。`);
    });
    ...
    describe('modify()メソッドのテスト', () => {
        ...
        it('存在しない商品を変更した場合、NotFoundExceptionがスローされる', async () => {
            ...
            await expect(usecase.modify(updateProduct)).rejects.toThrow(NotFoundException_1.NotFoundException);
            await expect(usecase.modify(updateProduct)).rejects.toThrow(`商品Id:(${updateProduct.id})の商品は存在しないため変更できませんでした。`);
        });
    });
});
